        ...
        <table className="w-full text-left text-sm">
          ...
          <tbody>
            {interactions.map((ix) => (
              <tr
                key={ix.id}
                onClick={() => router.push(`/interactions/${ix.id}`)}
              ...
              >
                ...
                <td className="px-6 py-3">
                  <span
                    className={`text-xs font-medium ${ix.direction === "in" ? "text-green-600" : "text-zinc-400"}`}
                  >
                    {ix.direction === "in" ? "⬅ In" : "➡ Out"}
                  </span>
                </td>
